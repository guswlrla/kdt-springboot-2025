...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
		...
		} catch(IOException e) { // 예외가 발생하면 null 리턴 -> unsuccessfulAuthentication 호출
			return null;
		}
		if(member == null) // member가 null이면 null 리턴 -> unsuccessfulAuthentication 호출
			return null;
		...
		// 자격 증명에 성공하면 Authentication 객체를 만들어서 리턴하면 successfulAuthentication 호출, 실패하면 unSuccessfulAuthentication 호출
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		User user = (User)authResult.getPrincipal(); // loadUserByUsername에서 만든 객체가 authResult에 담겨져 있음
		
		System.out.println("[JWTAuthenticationFilter]auth:" + user); // user 객체를 콘솔에 출력해서 확인
		
		String token = JWTUtil.getJWT(user.getUsername()); // username으로 JWT 생성, 이것은 예시로써 필요에 따라 정보 추가
		
		response.addHeader(HttpHeaders.AUTHORIZATION, token); // Response Header[Authorization]에 JWT를 저장해서 응답
	...
	protected void unsuccessfulAuthentication(HttpServletRequest request, HttpServletResponse response,
		...
		System.out.println("unsuccessfulAuthentication:" + failed); // 콘솔에 메시지 출력
