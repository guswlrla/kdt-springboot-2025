...
public class SecurityConfig {
	...
	PasswordEncoder passwordEncoder() { // Spring Security에 포함된 인터페이스
		return new BCryptPasswordEncoder(); // Spring Security에서 제공하는 암호화 클래스
	}
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.authorizeHttpRequests(security->security
				...
				.requestMatchers("/manager/**").hasAnyRole("MANAGER", "ADMIN") // manager, admin 중 하나라도 가지고 있으면 접근 가능
				.requestMatchers("/admin/**").hasRole("ADMIN") // admin만 접근 가능
				.anyRequest().permitAll()); // 위의 url을 제외한 모든 요청은 그냥 허용
		http.csrf(cf->cf.disable());
		// "/login" url을 사용자 정의 로그인 페이지로 사용
		// 로그인 성공 시 항상 "/loginSuccess" 페이지로 이동
		// true로 인해, 이전 페이지 기억 안하고 무조건 "/loginSuccess"로 이동
